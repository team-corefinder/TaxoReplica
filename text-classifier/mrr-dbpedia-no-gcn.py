...
from preprocessor_no_gcn import TaxoDataManager, DocumentManager
...
class Trainer:
    ...
    def evaluation(self, threshold):
            ...
            for i, train_data in enumerate(self.train_dataloader):
                ...
                if (i + 1) % 8 == 0:
                    print(
                        "[%5d]f1 accuracy : %.3f%% p1 accuracy: %.3f%% p3 accuracy: %.3f%% mrr accuracy: %.3f%%, num_data: %d"
                        % (
                            i + 1,
                            batch_accuracy * 100 / (self.B * 7 + predicted.shape[0]),
                            p1_batch_accuracy * 100 / (self.B * 7 + predicted.shape[0]),
            ...
            print(
                "[%s] f1 accuracy: %.3f%% ,p1 accuracy: %.3f%%, p3 accuracy: %.3f%%, mrr accuracy: %.3f%%"
                % (
                    self.data_name,
                    running_accuracy * 100 / self.data_size,
                    p1_running_accuracy * 100 / self.data_size,
                    p3_running_accuracy * 100 / self.data_size,
                    mrr_running_accuracy * 100 / self.data_size,
                )
            )
...
if __name__ == "__main__":
    ...
    taxonomy_file = 'taxonomy-acronym-fix.json'
    data_name = 'DBPEDIA'
    ...
    trainer.load_pretrained_model(f'{trainer.dir}trained/text-classifier-{trainer.data_name}-NoGNN-with-st.pt')
