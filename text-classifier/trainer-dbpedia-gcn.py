...
class Trainer:
    ...
    def load_pretrained_model(self, path=None):
        if path is None:
            path = f'{self.dir}trained/text-classifier-{self.data_name}-without-st.pt'
    ...
    def self_train(self, patience, threshold):
        ...
        for epoch in range(self.epoch):
            ...
            if running_accuracy > max_accuracy:
                self.save_model(
                  f'{self.dir}trained/text-classifier-{self.data_name}-with-st.pt')
...
if __name__ == "__main__":
    ...
    if is_train : 
        ...
        trainer.self_train(patience=3, threshold=0.95)
    else : 
        trainer.load_pretrained_model()
